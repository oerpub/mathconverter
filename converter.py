...
def _call_mathml_cloud(equation, mathtype):
    try:
        ...
        try:
            ...
            return 'null'   # bug in MathML Cloud. It sometimes returns 200 but no result (bummer).
...
def mathmlcloud2mathml(equation, iformat):
    ...
    if iformat == 'asciimath':
        ...
        mathtype = 'mml'
    # try x times to get MathML from web service:
    for i in range(1, 5):
...
def convert(equation, iformat, oformat):
    print
    if (iformat == oformat):
        sys.stderr.write('Nothing to convert here. Input and output format are the same!')
        sys.exit(1)
